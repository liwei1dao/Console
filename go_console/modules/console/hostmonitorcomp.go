...
func (this *HostMonitorComp) Init(service core.IService, module core.IModule, comp core.IModuleComp, options core.IModuleOptions) (err error) {
	...
	this.getCpuInfo()
	this.getMemoryInfo()
	this.hostMonitor = &pb.HostMonitor{
...
func (this *HostMonitorComp) getCpuInfo() {
	...
	this.cpuInfo = make([]*pb.CpuInfo, len(cpus))
	for i, cpu := range cpus {
		this.cpuInfo[i] = &pb.CpuInfo{
			CPU:        cpu.CPU,
...
func (this *HostMonitorComp) SaveMonitorData() {
	this.lock.Lock()
	defer this.lock.Unlock()
	cache.AddNewHostMonitor(this.hostMonitor)
	this.hostMonitor = &pb.HostMonitor{
		CpuUsageRate:    make([]float64, 60),
		MemoryUsageRate: make([]float64, 60),
	}
...
//读取主机监控数据
func (this *HostMonitorComp) GetHostMonitorData(queryTime pb.QueryMonitorTime) (result *pb.QueryHostMonitorDataResp) {
	this.lock.RLock()
	...
	Minute = time.Now().Minute()
	result = &pb.QueryHostMonitorDataResp{
	...
	if queryTime == pb.QueryMonitorTime_OneHour {
		step = 60 / chartleng
		totalMinute = 60
		hostMonitor = cache.GetHostMonitor(1)
	} else if queryTime == pb.QueryMonitorTime_SixHour {
		step = 60 / chartleng * 6
		totalMinute = 60 * 6
		hostMonitor = cache.GetHostMonitor(6)
	} else if queryTime == pb.QueryMonitorTime_OneDay {
		step = 60 / chartleng * 24
		totalMinute = 60 * 24
		hostMonitor = cache.GetHostMonitor(24)
	} else {
		step = 60 / chartleng * 24 * 7
		totalMinute = 60 * 24 * 7
		hostMonitor = cache.GetHostMonitor(24 * 7)
	}
