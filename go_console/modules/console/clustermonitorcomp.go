...
type ClusterMonitorComp struct {
	...
	clusterMonitor map[string]*pb.ClusterMonitor
}
...
func (this *ClusterMonitorComp) Init(service core.IService, module core.IModule, comp core.IModuleComp, options core.IModuleOptions) (err error) {
	...
	this.clusterMonitor = make(map[string]*pb.ClusterMonitor)
	...
	for _, s := range services {
		if result, err := this.service.RpcInvokeById(s.Id, monitor.Rpc_GetServiceMonitorInfo, true); err == nil {
			...
			if _, ok := this.clusterMonitor[s.Id]; !ok {
				this.clusterMonitor[s.Id] = &pb.ClusterMonitor{
...
func (this *ClusterMonitorComp) SaveMonitorData() {
	this.lock.Lock()
	defer this.lock.Unlock()
	cache.AddNewClusterMonitor(this.clusterMonitor)
	for k, _ := range this.clusterMonitor {
		this.clusterMonitor[k] = &pb.ClusterMonitor{
			CpuUsageRate:    make([]float64, 60),
			MemoryUsageRate: make([]float64, 60),
			GoroutineUsed:   make([]float64, 60),
			PreWeight:       make([]float64, 60),
...
func (this *ClusterMonitorComp) GetClusterMonitorDataResp(queryTime pb.QueryMonitorTime) (result map[string]map[string]interface{}) {
...
//读取主机监控数据
func (this *ClusterMonitorComp) getClusterMonitorData(service *core.ServiceMonitor, queryTime pb.QueryMonitorTime) (result *pb.ClusterMonitorData) {
	this.lock.RLock()
	...
	var (
		...
		clusterMonitor    []*pb.ClusterMonitor
		stepcpudata       [][]float64
	...
	if queryTime == pb.QueryMonitorTime_OneHour {
		step = 60 / chartleng
		totalMinute = 60
		clusterMonitor = cache.GetClusterMonitor(service.ServiceId, 1)
	} else if queryTime == pb.QueryMonitorTime_SixHour {
		step = 60 / chartleng * 6
		totalMinute = 60 * 6
		clusterMonitor = cache.GetClusterMonitor(service.ServiceId, 6)
	} else if queryTime == pb.QueryMonitorTime_OneDay {
		step = 60 / chartleng * 24
		totalMinute = 60 * 24
		clusterMonitor = cache.GetClusterMonitor(service.ServiceId, 24)
	} else {
		step = 60 / chartleng * 24 * 7
		totalMinute = 60 * 24 * 7
		clusterMonitor = cache.GetClusterMonitor(service.ServiceId, 24*7)
	}
